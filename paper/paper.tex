...
\citep{DBLP:journals/jsc/McKayP14} and the \citet{DBLP:journals/cacm/BronK73}
...
heuristic used selects the label class  with the smallest $\min(|\setG|,|\setH|)$ (a
surrogate for expected branching factor). From this class, a vertex is selected
from $\setG$ with maximum degree if $\graphH$ is sparse and with minimum degree if $\graphH$ is
dense.
...
However, we cannot solve the non-induced subgraph isomorphism this way, nor can
