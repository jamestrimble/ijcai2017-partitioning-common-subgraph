...
LARGELY FROM CIARAN'S EMAIL; NEEDS WORK:

\begin{algorithm}
\DontPrintSemicolon
\nl    \For {$q \in \N(b,G)$}{
\nl        remove $t$ from $D'_q$ \;
\nl        remove the inverse neighbourhood of $t$ from $D'_q$ \;
       }
\nl    \For {$q \in \N(b,\overline{G})$}{
\nl        remove $t$ from $D'_q$ \;
\nl        remove the neighbourhood of $t$ from $D'_q$ \;
       }
\caption{Replacement for \linerangeref{NewPWithEdge}{InnerLoopEnd} in CP algorithm}
\label{cpAlg}
\end{algorithm}

The first difference is in the underlying data structures: James uses a clever
partitioning data structure, not a conventional domain store. With the
algorithm described this way, I think the proof that domains are partitioned
into disjoint sets is almost immediate.

The second difference is in the bound function: the French code constructs a
maximum cardinality matching, with some special weighting trickery for $\bot$,
whilst James just sums the smaller of the two sides of each partition. If you
think a bit about what the matching looks like, you can see that these two give
the same answer, but at very different speeds.

The third difference is in the details of the variable ordering heuristic.
...
label class $\langle P',T' \rangle$ according to whether vertices are adjacent
to $v$ (resp. $w$), we now perform a four-way split where each vertex is
classified according to the label on its adjacency matrix from $v$ (resp. $w$).
...
graphs, along with graphs generated using several random models.  Pattern
graphs range from 4 vertices to 900 with a median of 80; target graphs range
from 10 vertices to 6671 with a median of 561.  Cumulative runtimes including
top-down algorithm from~\cite{UpcomingAAAIPaper} which was the previous state
of the art for this benchmark, are in \cref{figure:sip-cumulative}.  This is a
challenging set of instances, and more than half of the instances cannot be
solved within a timeout of 1000 seconds by any of the solvers. \McSplit\ is the
strongest solver overall for these instances.
