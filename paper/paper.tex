...
5.3.0. For comparison, we used the implementations of ?? cite clique, kdown,
CP.
...
?? kdown only does undirected unlabelled unconnected

?? There are five contours:
the clique encoding \citep{DBLP:conf/cp/McCreeshNPS16}, CP-FC
\citep{DBLP:conf/cp/NdiayeS11}, \McSplit\, AND TWO OTHERS.  WE NEED CITATIONS
FOR THESE AND ALSO NEED TO CLEAN UP THE LABELS ON THE GRAPHS.
...
in \cref{figure:plain-connected-cumulative} (again, ??kdown does not support the connected problem).  These results are very similar to
...
?? Let's put this hereish: I'VE RE-ADDED A SHORTER VERSION OF THIS
...
\FuncSty{McSplit} method once per goal size ($g, g-1, g-2, \dots$).  We
...
\citet{DBLP:conf/cp/NdiayeS11}, using an efficient, compressed representation
of domains.  In the CP-FC algorithm, each vertex in $v \in \V(G)$ is
represented by a variable, whose domain corresponds to the set of vertices in
$\V(H)$ to which $v$ may currently be mapped, with an additional special $\bot$
value representing an unmapped vertex.

Given a label class $\langle P,T \rangle$ in \McSplit\, the vertices in $P$
correspond to variables in CP-FC, all of which have domain equal to $T \cup \{
    \bot \}$.  The label-class representation of domains is possible because
throughout the CP-FC algorithm for max common subgraph, the domains of any two
variables are either identical or disjoint.  (To the best of our knowledge,
...
\McSplit\ requires $\BigO{g+h}$ time at each search node, and does not require
...
graph, and thus $\BigO{g^2h^2}$ in the order of the max common subgraph input
graphs.

?? Mention partition backtracking

?? Mention dual viewpoint heuristics
