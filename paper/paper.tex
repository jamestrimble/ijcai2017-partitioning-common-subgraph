...
\medskip
\nl $\AlgVar{bound} \gets |M|  + \sum_{\langle P,T \rangle \in \AlgVar{future}} \min(|P|,|T|)$ \label{CalcBound} \;
...
list of label classes, each represented as a $\langle P, T \rangle$ pair as
...
\Lineref{StoreIncumbent} stores the current mapping $M$ if it is larger than
the incumbent.  \Linerangeref{CalcBound}{PruneSearch} prune the search when a
calculated upper bound is not large enough to unseat the incumbent.
